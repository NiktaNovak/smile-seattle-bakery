...
function AdminDashboard() {
    ...
    useEffect(() => {
        const loadDashboard = async () => {
            setLoading(true);
            setError("");
            await Promise.all([getOrders(), getCakes()]);
            setLoading(false);
        };
        loadDashboard();
    }, [token]);
    ...
    const updateAvailability = async (cakeId, available) => {
        try {
            const response = await fetch(`${import.meta.env.VITE_API_URL}/api/cakes/${cakeId}/availability`,
                {
                    method: "PATCH",
                    headers: {
                        "Content-Type": "application/json",
            ...
            const data = await response.json();
            if (!response.ok) {
                throw new Error(data.error || "Unable to update cake availability.");
            }
            await getCakes();
        } catch (error) {
            console.error(error);
    ...
    return (
        <main className="admin-page">
            ...
            <section className="admin-section">
                ...
                {cakes.length === 0 ? (
                    ...
                    <div className="admin-cakes">
                        {cakes.map((cake) => (
                            <div className="admin-cake-card" key={cake.cake_id}>
                                <img src={cake.image_url} alt={cake.name} className="admin-cake-image" />
                                <div className="admin-cake-info">
                                    ...
                                    <button className="edit-cake-button" onClick={() => setEditingCake(cake)}>
                                        Edit
                                    </button>
                                    <button className="availability-button" onClick={() => updateAvailability(cake.cake_id, cake.available)}>
            ...
            {editingCake && (
                <section className="admin-section">
                    <EditCake cake={editingCake} onCakeUpdated={async () => { await getCakes(); setEditingCake(null); }} onCancel={() => setEditingCake(null)} />
                </section>
            )}
            <section className="admin-section">
